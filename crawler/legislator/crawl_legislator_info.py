...
def crawl_all(output_dir):
    crawl_info_pages(
        ...
        name="history_legislator_info",
        payload_base={"id": 16, "selectTerm": "all"},
        page_count=2,
    )
    crawl_info_pages(
        output_dir=output_dir, name="current_legislator_info", payload_base={"id": 9, "selectTerm": "all"}, page_count=1
    )
    crawl_info_pages(
        output_dir=output_dir, name="meeting_info", payload_base={"id": 42, "selectTerm": "all"}, page_count=6
    )
