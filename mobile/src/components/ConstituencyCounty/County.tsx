...
const sortDistrict = (districtArray: [DistrictInterface]) => {
    ...
    if (districtArray.length <= 1) {
        ...
        try {
            const sorted = Array.from(districtArray).sort(
                (a: DistrictInterface, b: DistrictInterface) => {
                    const districtNameA: string = a['name'];
                    const districtNameB: string = b['name'];

                    const matchedA = new RegExp(regularExpression, 'g').exec(
                        districtNameA
                    );
                    const matchedB = new RegExp(regularExpression, 'g').exec(
                        districtNameB
                    );
...
const RegionInfoRow = (props?: any) => {
    ...
    for (let i = 0; i < columnNum; i++) {
            ...
            if (partyName) {
                ...
                const backgroundColor =
                    partyColor !== undefined
                        ? partyColor['main']
                        : exsitingPartyButColorNotFound;
                const existingPartyStyle: React.CSSProperties = {
                    background: backgroundColor
                };

                return existingPartyStyle;
            } else {
                const emptyPartyStyle: React.CSSProperties = {
                    visibility: 'hidden'
                };
        ...
        cells.push(
            <div
                style={cellStyle}
                className={classes.regionInfoCell}
...
const RegionInfo = (props?: any) => {
    ...
    const rowNum = Math.max.apply(
        Math,
        infoData.map((d: any) => d['party'].length)
    );
    ...
    for (let i = 0; i < rowNum; i++) {
        ...
        rows.push(
            <div
                style={rowStyle}
                className={classes.regionInfoCellRow}
                key={`row-${i}`}
            >
                <RegionInfoRow
                    rowData={rowData}
    ...
    return <div className={classes.regionInfo}>{rows}</div>;
};
...
const Constituency = () => {
    ...
    const [regionInfoData, setRegionInfoData] = useState<
        regionInfoDataInterface
    >(defaultRegionInfoData);
    ...
    const [
        ...
        refListItemContainerMeasure
    ] = useListItemContainerWidth();
    ...
    return (
        <Container className="p-0">
            ...
            <List className={classes.listItemWrapper}>
                {isDesktop && (
                    <div className={classes.listItemContainerMeasureWrapper}>
                        <div
                            ref={refListItemContainerMeasure}
                            className={classes.listItemContainerMeasure}
                        >
                            {(() => {
                ...
                <div
                    style={{ width: `${listItemContainerWidth}px` }}
                        ...
                        >
                            ...
                            {isDesktop &&
                                ...
                                regionInfoData[county][0]['party'][0] !==
                                    ...
                                    <RegionInfo
                                        classes={classes}
                                        infoData={regionInfoData[county]}
                                    />
                                )}
                        </ListItem>
                    ))}
