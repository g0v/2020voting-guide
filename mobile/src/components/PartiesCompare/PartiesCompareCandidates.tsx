...
export default function PartiesCompareCandidates({ name }: Props) {
    const candidates: SingleCandidateInParty[] = take((partyCandidates as PartyCandidates)[name], 7);
    return (
        <div className="candidate-compare-col parties-compare-candidates">
            <div className="h4 color-black mb-3 font-weight-500">本屆不分區提名</div>
            {candidates.map((candidate: SingleCandidateInParty, idx: number) => {
                const { name, age, experience } = candidate;
                return (
                    <div className="parties-compare-candidates-item mb-3" key={name}>
                        <div className="d-flex align-items-center">
                            <span className="h3 color-red mr-2">{idx + 1}</span>
                            <span className="h4 mr-2 color-real-black">{name}</span>
                            {age && <div className="h5 color-gray">{age}歲</div>}
                        </div>
                        <div className="h5 color-gray text-overflow-1">{experience.split(/。|\n|，/)[0]}</div>
                    </div>
                );
            })}
