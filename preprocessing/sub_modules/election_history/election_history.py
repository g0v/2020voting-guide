...
BULLETIN_FILE_PATH = f'{FILE_DIR}/../../../data/organized/2016-bulletin.json'
...
def readRawData():
    history_info = {}
    bulletin_info = {}
    with open(BULLETIN_FILE_PATH, "r") as f:
        bulletin_info = json.load(f)
        ...
        if len(repm_files) > 0:
            ...
            with open(cand_filename) as cf:
                ...
                for row in reader:
                    row["party"] = party_mappings[re.sub(r"\D", "", row["partyNo"].strip())]  # strip() to prevent some dirty data like "1 "
                    row["electionName"] = "-".join(dir.split("voteData/")[1].split("/"))
                    if row["electionName"].startswith("2016") and row["name"] in bulletin_info:
                        row["bulltein"] = bulletin_info[row["name"]]
                    history_info.setdefault(row['name'], []).append(row)
    return history_info
